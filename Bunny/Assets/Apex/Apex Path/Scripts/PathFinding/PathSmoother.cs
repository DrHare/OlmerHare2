...
namespace Apex.PathFinding
{
    ...
    public class PathSmoother : ISmoothPaths
    {
        ...
        /// </returns>
        public Path Smooth(IPathNode goal, int maxPathLength, IPathRequest request)
        {
        ...
        private static bool CanReducePath(IPositioned point1, IPositioned point3, AttributeMask requesterAttributes, float requesterRadius, CellMatrix matrix)
        {
            ...
            var startCost = startCell.cost;

            var adjusted = matrix.GetCell(p1 - radiusAdjust, false);
            if (adjusted != null)
            {
                startCell = adjusted;
            }
            ...
            var endCost = endCell.cost;

            adjusted = matrix.GetCell(p3 + radiusAdjust, false);
            if (adjusted != null)
            {
                endCell = adjusted;
            }
            ...
            for (int x = startCell.matrixPosX; x <= endCell.matrixPosX; x++)
            {
                ...
                for (int z = startZ; z != endZ; z += incZ)
                {
                    ...
                    if (!intermediary.isWalkableFrom(startCell, requesterAttributes) || (startCost < intermediary.cost && endCost < intermediary.cost))
                    {
                        return false;
                    }
                }
