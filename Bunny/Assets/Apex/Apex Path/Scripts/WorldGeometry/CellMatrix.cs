...
namespace Apex.WorldGeometry
{
    ...
    public class CellMatrix : Matrix<Cell>, IHeightMap
    {
        ...
        private void UpdateCellHeightData(Cell reference, Cell neighbour, NeighbourPosition neighbourPos, float maxHeightDiff, bool testForPermanentBlock)
        {
            ...
            var offsetBase = Mathf.Min(_cellSize / 2f, _obstacleSensitivityRange);
            var offsets = GetPerpendicularOffsets(dir, offsetBase);
            ...
            for (int o = 0; o < 3; o++)
            {
                ...
                for (int i = 0; i < steps; i++)
                {
                    ...
                    if (heightDiff > maxHeightDiff)
                    {
                        ...
                        {
                            if (testForPermanentBlock && distFromCenter < offsetBase)
                            {
        ...
        private Vector3[] GetPerpendicularOffsets(VectorXZ dir, float offsetBase)
        {
            Vector3 ppd;
            
            if (dir.x != 0 && dir.z != 0)
            {
                var offSet = offsetBase / SquareRootTwo;
                ppd = new Vector3(offSet * -dir.x, 0.0f, offSet * dir.z);
            }
            else
            {
                ppd = new Vector3(offsetBase * dir.z, 0.0f, offsetBase * dir.x);
            }
